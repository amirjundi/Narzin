...
class MerchantAuthCubit extends Cubit<MerchantAuthState> {
  ...
  prepareMerchantDetails(VendorLoginModel model){
    address = TextEditingController(text: model.data?.vendorDetails?.address);
    phone = TextEditingController(text: model.data?.vendorDetails?.phone);
    arabicName = TextEditingController(text: model.data?.vendorDetails?.storeNameInArabic);
    englishName = TextEditingController(text: model.data?.vendorDetails?.storeNameInGerman);
    category = TextEditingController(text: model.data?.vendorDetails?.storeType);
  }
...
// - merchantRegisterModel and MerchantRegisterModel.fromJson(Map<String, dynamic> json)
  Future<String?> updateMerchant({required String token,required String id}) async {
    String apiUrl = '${Constants.apiBaseUrl}vendors/$id';
  ...
  Future<String?> deleteMerchant({required String token,required String id}) async {
    String apiUrl = '${Constants.apiBaseUrl}vendors/$id';
    ...
    try {
      ...
      if (responseData['status'] == true) {
        ...
        errorMessage += responseData['errors'] != null
          ...
          Helpers.showColoredToast(
              color: Colors.red,
              message: 'Got an error: $errorMessage'
          );
          return errorMessage;
        // Helpers.showColoredToast(color: Colors.red, message: errorMessage);
