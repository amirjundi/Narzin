...
5. **`product_rail`** — `title` (i18n), `rule` (`newest` = latest created active products | `best_sellers` = most units sold from order items, all-time | `biggest_discount` = highest current discount % | `category` with `category_id` | `manual` with `product_ids[]`), `limit` (default 12, max 24).
